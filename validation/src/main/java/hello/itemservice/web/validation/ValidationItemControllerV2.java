...
public class ValidationItemControllerV2 {
    ...
    //@RequiredArgsConstructor가 생성자 자동으로 생성해줌
    //생성자가 하나이기 때문에 @Autowired 생략 가능
    private final ItemRepository itemRepository;
    private final ItemValidator itemValidator;

    //프로퍼티 바인딩 작업 전에 자동으로 호출된다.
    @InitBinder
...
//    @PostMapping("/add")
    public String addItemV1(@ModelAttribute Item item, BindingResult bindingResult, RedirectAttributes redirectAttributes, Model model) {
        ...
        if(item.getPrice() == null || item.getPrice() < 1000 || item.getPrice() > 1000000) {
            bindingResult.addError(new FieldError("item", "price", "가격은 1,000 ~ 1,000,000 사이입니다"));
        }
...
//    @PostMapping("/add")
    public String addItemV2(@ModelAttribute Item item, BindingResult bindingResult, RedirectAttributes redirectAttributes, Model model) {
        ...
        if(item.getPrice() == null || item.getPrice() < 1000 || item.getPrice() > 1000000) {
            bindingResult.addError(new FieldError("item", "price", item.getPrice(), false, null, null, "가격은 1,000 ~ 1,000,000 사이입니다"));
        }
        if(item.getQuantity() == null || item.getQuantity() > 9999) {
